...
def double_fig(data_df, os_dict, ax=None, opt=2, legend=False, display=True):
    ...
    # Plot 1
    ax[0].plot(data_df.t/3600, [HI] * len(data_df), 'k', alpha=0.5, label='Ice surface')
    ax[0].plot(data_df.t/3600, [Hb] * len(data_df), 'b:', alpha=0.5, label='Brine level')
    ax[0].plot(data_df.t/3600, [HD] * len(data_df), 'b', label='Water level')
    ax[0].plot(data_df.t/3600, [HI-Hb] * len(data_df), label='H$_i$ - H$_b$')
    ...
    ax[0].fill_between(data_df.t/3600, [-HR-0.1] * len(data_df), data_df[['h_R', 'h_co']].max(axis=1), color='b', alpha=0.5)
    ...
    if opt == 2:
        ...
        ax[1].set_ylabel('Presssure differential ')
    ...
    if legend:
        ax[0].legend(l_, h_, loc='lower center', ncol=3, labelspacing=0.2, columnspacing=1,
                   bbox_to_anchor=(0, 0, 1, 1),
                   fancybox=False, shadow=False, frameon=False)
        if opt == 2:
            ax[1].legend(l2_, h2_, loc='lower center', ncol=3, labelspacing=0.2, columnspacing=1,
                        bbox_to_anchor=(0, 0, 1, 1),
                        fancybox=False, shadow=False, frameon=False)
...
for ii_ci, ci in enumerate(os_CI):
    ...
    ax[ax_r, ax_c].text(0, 0.75*os_dict['HI'], str('h$_o$=%.02f cm' % (data_df.loc[data_df.t.notna(), 'h_o'].max()*100)))
