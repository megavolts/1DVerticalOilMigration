...
R = 2e-3  # True to enable, if float
HR = 0.9
VR = 300
...
while os.path.exists(run_fp) and os.path.exists(ps_fs):
    run_n += 1
    run_fn = run_name + str('run_%04i-N%i_ts%.1f' % (run_n, N_layers, t_step))
    run_fp = os.path.join(run_fd, run_fn)
    ps_fn = run_fn + '-porespace.jpg'
    ps_fs = os.path.join(run_fd, ps_fn)
...
while not STOP:
    ...
    dvbs = dvb
    kk_hb = ii_hb
    while dvbs > 0 and kk_hb < N_layers-1:
        ...
        # TODO potentially replace by np.abs(f_b_kk -1) < epsilon
        if f_b_kk - 1 > -epsilon:
